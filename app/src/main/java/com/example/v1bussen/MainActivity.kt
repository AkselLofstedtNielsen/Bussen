...
class MainActivity : AppCompatActivity() {
    ...
    var firstRowCard = 0 //int of value for card picked in first row
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bottomCard1.setOnClickListener {
            var random = (1..52).shuffled()
                .first()//shuffled 1..52 int so its "random" but usable again inside the {}
            bottomCard1.setImageResource(deck[random].image)
            disableFirstRow()
            firstRowCard = deck[random].value
        }
    ...
    fun checkValueNextRow() {

        if (firstRowCard != 0){//check value in rows
